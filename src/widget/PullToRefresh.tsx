...
export default class PullToRefresh extends Component<Props, State> {
    ...
    onPanResponderMove(event: GestureResponderEvent, gestureState: PanResponderGestureState) {
        //设置刷新头部的高度
        if (gestureState.dy >= 0 && gestureState.dy <= this.props.headerHeight) {
            // const dy = Math.max(0, gestureState.dy);
            this.state.containerTop.setValue(gestureState.dy);
        } else if (gestureState.dy > this.props.headerHeight) {
            //禁止header无限下滑，滑动距离超出header高度时，设置容器头部的偏移量为header的高度
            this.state.containerTop.setValue(this.props.headerHeight);
        } else {
            if (this.scrollRef) {
